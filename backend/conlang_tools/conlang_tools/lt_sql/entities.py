...
db_conlang_defs = Table(
    "conlang_defs",
    Base.metadata,
    Column(
        "db_id",
        BigInteger,
        ForeignKey("synsets.db_id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    ),
    Column(
        "word_id",
        BigInteger,
        ForeignKey("conlang_words.word_id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    ),
    PrimaryKeyConstraint("db_id", "word_id"),
)

db_synset_associations = Table(
    "synset_relations",
    Base.metadata,
    Column(
        "db_id_1",
...
class DbLangConfig(Base):
    ...
    conlang_words: Mapped[List["DbConlangWord"]] = relationship(
        back_populates="lang_config"
    )


class DbConlangWord(Base):
    ...
    lang_config_id: Mapped[int] = mapped_column(
        ForeignKey(
            "lang_configs.lang_config_id", onupdate="CASCADE", ondelete="CASCADE"
        ),
        primary_key=True,
    )

    lang_config: Mapped["DbLangConfig"] = relationship(
        back_populates="conlang_words"
    )
    synsets: Mapped[List[DbSynset]] = relationship(
        secondary=db_conlang_defs, back_populates="conlang_words"
    )
...
class DbSynset(Base):
    ...
    conlang_words: Mapped[List[DbConlangWord]] = relationship(
        secondary=db_conlang_defs, back_populates="synsets"
    )


class DbLexicon(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("lang_config_id", "word_id", "db_id"),)


ENTITY_MAPPING: dict[str, Base] = {
    "lang_config": DbLangConfig,
    "conlang_word": DbConlangWord,
    "synset": DbSynset,
    "lexicon": DbLexicon,
    "base": Base,
    "conlang_defs": db_conlang_defs,
    "synset_relations": db_synset_associations,
}
